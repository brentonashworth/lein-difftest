...
lein-difftest is a Leiningen plugin for running <code>clojure.test</code> tests. It works exactly the same as "lein test" but with alternate test reporting. When a test fails, the report shows the diff of the actual and expected forms. When a test fails due to an error, stacktraces are cleaned up with clj-stacktrace.
...
For Clojure 1.2 use <code>[lein-difftest "1.3.3"]</code>
...
This plugin uses Robert Hooke to change the behavior of the Leiningen test task. If you want to go back to standard test output then you will need to remove the entry from the :hooks list in project.clj.
